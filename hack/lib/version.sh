...
kube::version::ldflags() {
  ...
  if [[ -n ${KUBE_GIT_COMMIT-} ]]; then
    ldflags+=($(kube::version::ldflag "Revision" "${KUBE_GIT_COMMIT}"))
    ldflags+=($(kube::version::ldflag "Branch" "${KUBE_GIT_TREE_STATE}"))
  fi

  if [[ -n ${KUBE_GIT_VERSION-} ]]; then
    ldflags+=($(kube::version::ldflag "Version" "${KUBE_GIT_VERSION}"))
  fi
